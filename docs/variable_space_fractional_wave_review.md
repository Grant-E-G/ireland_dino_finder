...
Hanyga's analysis sharpens this: for power-law attenuation, complete monotonicity of the relaxation modulus holds only for `1/2 ≤ α ≤ 1`, which is a real constraint on which exponents are physically admissible rather than merely fitted.
...
One framing caveat before the specifics: much of the cited numerical-analysis literature concerns *subdiffusion*, `0 < α < 1`. The wave models here live in the `1 < α < 2` regime (and the Zener form pushes effective orders past 2). The qualitative lessons transfer, but the regularity and stability details do not transfer automatically, and the relevant references differ. Jin-Lazarov-Zhou and Lewandowska-Kosztolowicz are subdiffusion results; Li-Wang-Xie is the in-regime fractional-*wave* nonsmooth-data analysis. It is worth tagging each warning by the regime it was proved in.
